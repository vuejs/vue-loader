...
class VueLoader {
  constructor (filePath, options) {
    ...
    this.loaders = Object.assign({}, defaultLoaders, options.loaders)
  }
  ...
  render (content, loaderContext) {
    ...
    if (parts.styles.length) {
      ...
      parts.styles.forEach(function (style, i) {
        ...
        var requireString = style.src
          ? this.getRequireForImport('styles', style, style.scoped)
          : this.getRequire('styles', style, i, style.scoped)
    ...
    if (script) {
      ...
      output +=
        '__vue_exports__ = ' + (
          script.src
            ? this.getRequireForImport('script', script)
            : this.getRequire('script', script)
        )
    }
    ...
    if (template) {
      ...
      output += 'var __vue_template__ = ' + (
        template.src
          ? this.getRequireForImport('template', template)
          : this.getRequire('template', template)
      )
  ...
  getRequireForImport (type, impt, scoped) {
    return 'require(' +
...
module.exports = function (content) {
  ...
  var query = loaderUtils.parseQuery(options.query)
  ...
  return vueLoader.render(content, loaderContext)
}
