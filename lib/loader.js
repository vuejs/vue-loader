...
module.exports = function (content) {
  ...
  const templateCompilerOptionsObject = {
    hasScoped,
    hasComment,
    buble: bubleTemplateOptions
  }
  if (hasScoped) {
    templateCompilerOptionsObject.id = moduleId
  }
  const templateCompilerOptions = '?' + JSON.stringify(templateCompilerOptionsObject)
  ...
  if (isShadowMode) {
    ...
    output +=
      ...
      `;(window[shadowInjectId] || (window[shadowInjectId] = [])).push(injectStyle)\n`
  }
  ...
  function getRawLoaderString (type, part, index, scoped) {
    ...
    if (type === 'styles') {
      ...
      const styleCompilerOptionsObject = {
      ...
      if (scoped) {
        styleCompilerOptionsObject.id = moduleId
      }
      styleCompiler =
        styleCompilerPath +
        '?' +
        JSON.stringify(styleCompilerOptionsObject) +
        '!'
