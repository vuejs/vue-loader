...
module.exports = function (content) {
  ...
  function getLoaderString (type, part, scoped) {
    ...
    var opt = part.opt
    if (opt) { // to resolve a possible relative given file
      try {
        JSON.parse(opt) // if it is valid JSON, no resolving is needed
      } catch (e) {
        var newOpt = path.resolve(path.dirname(filePath), opt)
        try {
          var stats = fs.lstatSync(newOpt)
          if (stats.isFile()) {
            opt = newOpt
          }
        } catch (e) {}
