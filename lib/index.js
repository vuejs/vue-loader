...
module.exports = function (source) {
  ...
  if (incomingQuery.type) {
    if (incomingQuery.type === 'custom' && descriptor.customBlocks.length === 0) {
      // #1344, if a custom block contains "src" attribute pointing to SFC, we need
      // to disable "selectBlock" when the target SFC is loading.
    } else {
      return selectBlock(
        descriptor,
        loaderContext,
